...
const columns = [
  ...
  { accessorKey: "fecha_creacion", header: "Creación" },
...
const TablaCategorias = () => {
    ...
    const fetchCategorias = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `https://api-fake-sport.onrender.com/api/categorias?page=${pageIndex + 1}&limit=${pageSize}`,
          {
            method: "GET",
            headers: {
  ...
  return (
    <div className="container mt-4">
      ...
      {!loading && !error && data.length === 0 && (
        <p className="alert alert-warning">No hay categorías disponibles.</p>
      )}
    </div>
  );
};
